# -*- coding: utf-8 -*-
"""
compare different passive global attacks under defense with --defense parameter, support following:
...
def mia_compare(server_models, target_client_models, train_loader, test_loader, args):
    if not dropout:
        ...
        # target_client_models = [model for model in target_client_models if model is not None]
    ...
    if dropout:
        logging.debug('passive global CS-MIA dropout_p {}: acc {}, pre {}, recall {}, f1 {}'.format(args.dropout_p, acc, pre, recall, f1))
    else:
